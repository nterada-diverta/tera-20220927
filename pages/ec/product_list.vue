...
export default {
    ...
    async asyncData ({ $axios, params }) {
        try {
            const profile = await $axios.$get('/rcms-api/6/profile')
            let cartItems = {
                details: null
            }
            if (profile.ec_cart_id) {
                cartItems = await $axios.$get('/rcms-api/6/cart-items/' + profile.ec_cart_id)
            }
            const productList = await $axios.$get('/rcms-api/6/product-list')
    ...
    methods: {
        async addCart(product_id) {
            const profile = await this.$axios.$get('/rcms-api/6/profile')
            const addItem = {
            ...
            const addCartResp = await this.$axios.$post('/rcms-api/6/add-cart', addItem)
            if (addCartResp.errors.length == 0) {
                alert('Success add cart')
                const cartListResp = await this.$axios.$get('/rcms-api/6/cart-items/' + profile.ec_cart_id)
